...
class ProductViewModel (var navController: NavHostController,
                        var context: Context)
{
    val cloudinaryUrl = "https://api.cloudinary.com/v1_1/dshw4d5se/image/upload"
    val uploadPreset = "paidproducts"
    val databaseReference by lazy { FirebaseDatabase.getInstance().getReference("Products") }
    // Fetch  all Products  from firebase  database function
    fun allProducts(
    ...
    fun singleProduct(productId: String, product: MutableState<Product>) {
        databaseReference.child(productId).addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                val retrievedProduct = snapshot.getValue(Product::class.java)
                if (retrievedProduct != null) {
                    product.value = retrievedProduct
                }
            }
